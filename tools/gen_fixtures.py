...
Requires the Python `attenu-guard` package (>=0.6, which ships the interop vectors this
...
What lands in `test/fixtures/`:
  ...
  *.bundle.json            clean (HS256 and Ed25519 anchors) plus one file per tamper
  ...
  vectors/*.json           the Internet-Draft's delegation-chain interop vectors, copied
                           ...
                           and eight that must each be rejected for a named reason
"""
...
try:
    ...
    raise SystemExit(
        ...
        "Upgrade with:  pip install --upgrade 'attenu-guard>=0.6'"
    ) from e
...
def canonical_vectors() -> list[dict]:
    ...
    `source` is written with Python's DEFAULT separators — the form the `.jsonl` ledger
    uses on disk — so a reader has to parse it and re-serialise rather than pattern-match
    the text. The float cases are the ones a naive `JSON.stringify` gets wrong.
    """
    values = [
        {},
        [],
...
def chain_vectors() -> dict[str, str]:
    ...
    One valid Delegation Chain and eight adversarial ones, each naming the reason it
    must be rejected FOR. They are generated by the Python repository's
...
def main() -> None:
    ...
    # A ledger exactly as Python writes it to disk (default separators, one JSON per line).
    root = build_run()
    entries = root.audit_log().entries
    write("ledger.jsonl", "".join(json.dumps(e, sort_keys=True) + "\n" for e in entries))
    ...
    write("tampered_ledger.jsonl", "".join(json.dumps(e, sort_keys=True) + "\n" for e in edited))
