 ...
 * Delegation Chain with no call to an authorization server. The seven interop
...
import { RawNumber, parseJson, toPlain, type CJson, type Json } from "./canonical.js";
...
interface ParsedToken {
   ...
   * to match Python, where a float there is not an integer and the token is
   * refused.
   */
...
function parseToken(token: string): ParsedToken {
  ...
  let sig: Buffer;
  try {
    header = toPlain(parseJson(b64urlDecode(headerB64).toString("utf8")));
    raw = parseJson(b64urlDecode(payloadB64).toString("utf8"));
    sig = b64urlDecode(sigB64);
  } catch (e) {
    throw new WireError(WireReasonCode.MALFORMED, `could not decode token: ${String(e)}`);
  }
  const payload = toPlain<Json>(raw);
  ...
  return {
    header,
    payload,
    payloadRaw: raw as Record<string, CJson>,
    sig,
